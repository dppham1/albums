...
class UserTestCase(unittest.TestCase):
    ...
    def test_get_albums(self, patch_albums, patch_album_schema):
        patch_albums.query.filter_by.return_value.order_by.return_value.all.return_value = [
            Albums(
                id=ANY,
                artist_id=ANY,
                genre_id=ANY,
        ...
        self.assertAlmostEqual(response.json, {"album_data": True})
    ...
    def test_create_album_success(
        self,
        patch_album_schema_load,
        patch_album_schema_dump,
        patch_db,
        patch_jwt_decode,
        patch_users,
    ):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json, {"album_data": True})
    ...
    def test_create_album_fail(self, patch_jwt_decode, patch_users):
        ...
        response = self.client.post(
            "/api/albums/",
            data=json.dumps(data_without_genre_id),
            headers={
                "Content-Type": "application/json",
                "Authorization": "Bearer test_token",
            },
    ...
    def test_update_album_success(
        self, patch_jwt_decode, patch_users, patch_album_schema, patch_db
    ):
        patch_jwt_decode.return_value = {"id": "123"}
        patch_users.query.filter_by.return_value.first.return_value = True
        patch_album_schema.query.filter_by.return_value.first.return_value = Albums(
            id=18,
            name="Griff Album3",
            images=["https:www.google.com"],
            song_count=5,
            ...
            artist_id=1,
            genre_id=1,
            release_date=datetime(1998, 2, 22, 0, 0),
            created_at=datetime(2023, 3, 14, 6, 53, 8),
            updated_at=datetime(2023, 3, 14, 6, 53, 8),
        )
        patch_db.commit.return_value = None
    ...
    def test_update_album_fail(
        ...
        response = self.client.put(
            "/api/albums/1",
            data=json.dumps(data_without_label_rights),
            headers={
                "Content-Type": "application/json",
                "Authorization": "Bearer test_token",
            },
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(
            response.json.get("label_rights"), ["Missing data for required field."]
        )
    ...
    def test_delete_album_success(
        self, patch_jwt_decode, patch_users, patch_albums, patch_db
    ):
        patch_jwt_decode.return_value = {"id": "123"}
        patch_users.query.filter_by.return_value.first.return_value = True
        patch_albums.query.filter_by.return_value.first.return_value = {
            "artist_id": 1,
            "genre_id": 1,
            "images": ["https:www.google.com"],
            "label_rights": "Sony Records",
